...
let scene, camera, renderer, cube, controls, draughts, board, mouse, raycaster, selectedPiece = null;
 ...
 function init() {
   ...
   loader.load("./Checker.glb", function (glft){
       const checkerMesh = glft.scene.children.find((child) => child.name === "Checker");
...
function addCheckers(checkerMesh){
    for (let i = 0; i < 51; i++) {
        ...
        const squarePosition = positionSquare(i);
        if (pieceOn === 'b') {
...
function onClick(event){
    ...
    if (selectedPiece) {
        ...
        if (intersects.length > 0 && intersects[0].object.userData.squareNumber) {
            ...
            const targetPosition = positionSquare(targetSquare);
